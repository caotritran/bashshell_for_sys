...
for domain in /home/*/domains/*; do
    if [ -d "${domain}" ]; then #If a directory
        ...
        echo "- "$domain;
        zip -r $BACKUP_DIR/${domain}.zip /home/*/domains/$domain/* -q -x '/home/*/domains/$domain/public_html/wp-content/cache/*' -x '/home/*/domains/$domain/public_html/wp-content/uploads/backupbuddy_backups/*' #Exclude cache
        echo "Finished";
