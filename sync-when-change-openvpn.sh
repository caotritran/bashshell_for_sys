#/bin/bash
#Author: tritran
#Script check change file to sync from Server Openvpn01 to Server Openvpn02
 
md5=`md5sum /etc/openvpn/easy-rsa/pki/index.txt | awk '{print $1}'`
#RSYNC_PASSWORD='my_pass'
...
if [ "$md5" != "$md5_before" ]; then
    ...
    rsync -avzh -e 'ssh -i /root/.ssh/openvpn02' /etc/openvpn/ root@45.252.251.121:/etc/
    if [ $? -eq 0 ]; then
